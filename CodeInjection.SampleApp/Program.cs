...
namespace CodeInjection.SampleApp
{
	class Program
	{
		...
		private static void StartLoadTest(ILogicInjector codeInjector, ISampleService sampleService, IInjectedPipeline pipeline) {
			var oldOut = Console.Out;
			Console.SetOut(new StringWriter());
			var stopWatch = new Stopwatch();
			stopWatch.Start();
			for (var i = 0; i < 1000; i++) {
				CallService(codeInjector, sampleService, pipeline, "test");
			}
			stopWatch.Stop();
		...
		private static void CallService(ILogicInjector codeInjector, ISampleService sampleService, IInjectedPipeline pipeline, string clientName) {
			var decoratedService = codeInjector.CreateProxyFor(sampleService, pipeline);
			decoratedService.DoSomethingUseful(clientName);
		}

		private static void StandartInvokation(ILogicInjector codeInjector, ISampleService sampleService, IInjectedPipeline pipeline) {
			Console.WriteLine();
